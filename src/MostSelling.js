...
const MostSellingProductList = () => {
    ...
    const fetchMostSelling = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/products/most-selling');
            console.log('Fetched most selling:', response.data);
    ...
    const handleMostSellingClick = () => {
        navigate('/most-selling');
    };
    ...
    return (
        <div className="container my-5" style={{ background: "linear-gradient(to right, #ccdcf1,#091b33)", position: 'relative'}}>
            <h2
    className="mb-4"
    onClick={handleMostSellingClick}
    style={{
        cursor: 'pointer',
...
export default MostSellingProductList;
